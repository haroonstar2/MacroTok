...
export default function RecipeCard({
  ...
  const handleConfirmSave = async (e) => {
    ...
    try {
      ...
      setScheduledEntries((prev) => [
        ...prev,
        { date: newEntry.date, slot: newEntry.slot },
      ]);
      setIsScheduled(true);
      setShowDatePicker(false);
      setShowAddNew(false);
  ...
  return (
    <div
      className="card-link"
      onClick={() => navigate(`/recipe/${recipeDetails.id}`)}
    >
      <article className="card">
              ...
              {isScheduled && (
                <div style={{ marginBottom: 8 }}>
                  ...
                  {loadingEntries ? (
                    ...
                    <p style={{ fontSize: 12, margin: 0, color: "#888" }}>
                      No entries found.
                    </p>
                  ) : (
              ...
              {(!isScheduled || showAddNew) && (
                <div style={{ marginTop: isScheduled ? 6 : 0 }}>
                  ...
                  <select
                    value={newEntry.slot}
                    onChange={(e) =>
                      setNewEntry({ ...newEntry, slot: e.target.value })
                    }
                    style={{
                      padding: 5,
                      borderRadius: 4,
        ...
        <div className="card-body">
          ...
          <div className="stats">
            <StatBar
              label="Protein"
              value={recipeDetails.protein}
              unit="g"
              variant="protein"
            />
            <StatBar
              label="Carbs"
              value={recipeDetails.carbs}
              unit="g"
              variant="carbs"
            />
            <StatBar
              label="Fats"
              value={recipeDetails.fats}
              unit="g"
              variant="fats"
            />
          </div>
