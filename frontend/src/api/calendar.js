 ...
 */
export async function getIsRecipeScheduled(recipeId, dateISO, slot = "recipes") {
 ...
 */
export async function removeRecipeFromDate(recipe, dateISO, slot = "breakfast") {
  const user = auth.currentUser;
  if (!user) throw new Error("User must be logged in.");
  ...
  const storedRecipe = storedSlot.find(r => String(r.id) === String(recipe.id));
  ...
  try {
    ...
    snapshot.forEach(docSnap => {
      ...
      ['breakfast', 'lunch', 'dinner'].forEach(slot => {
        if (Array.isArray(data[slot]) && data[slot].some(r => String(r.id) === String(recipeId))) {
